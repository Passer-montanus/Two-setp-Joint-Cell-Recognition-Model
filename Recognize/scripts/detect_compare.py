...
def parse_args():
    ...
    parser.add_argument('--iou_off', action='store_false', dest='draw_iou', help='禁用IoU数值绘制到图片上') # 输入以激活禁用
    parser.add_argument('--class_off', action='store_false', dest='draw_class',help='禁用class绘制到图片上') # 输入以激活禁用
    ...
    parser.add_argument('--img_path', type=str, default='D:/yolov8/Heatmap/dataset/org', help='原始图像路径')
    parser.add_argument('--label_path', type=str, default='D:/yolov8/Heatmap/dataset/labels', help='标签路径')
    parser.add_argument('--predict_path', type=str, default='D:/yolov8/Heatmap/runs/Heatmap/labels', help='预测结果路径')
    parser.add_argument('--save_path', type=str, default='D:/yolov8/Heatmap/dataset/save_pic', help='保存图像的路径')
    parser.add_argument('--result_file_path', type=str, default='D:/yolov8/Heatmap/dataset/result.txt', help='结果文件路径')
...
detect_color, missing_color, error_color  = (0, 255, 0), (0, 0, 255), (255, 0, 0) # 绿色(检测正确)、红色(漏检)、蓝色(误检)

# 指定args变量
mode = args.mode
# iou_off = args.iou_off
iou_off = args.draw_iou
class_off = args.draw_class
...
draw_options = []
if args.draw_class:
    draw_options.append("class")
if args.draw_iou:
    draw_options.append("iou")
options_suffix = "_".join(draw_options) if draw_options else "no_labels"

save_path = os.path.join(args.save_path, args.mode, options_suffix)
...
def draw_box(img, box, color, class_name=None, iou_value=None, 
    ...
    if class_name is not None and args.draw_class==True: # 增加判断是否输出class
        text = class_name
    ...
    if iou_value is not None and args.draw_iou==True: # 增加判断是否输出iou
