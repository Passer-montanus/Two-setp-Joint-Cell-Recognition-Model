...
#     thickness = 3  # 增加方框的厚度
#     font_scale = 0.7  # 文本的比例尺

#     x1, y1, x2, y2 = int(box[0]), int(box[1]), int(box[2]), int(box[3])
#     cv2.rectangle(img, (x1, y1), (x2, y2), color, thickness=thickness)

#     # 输出分类名称和IoU数值
#     if class_name is not None and iou_value is not None:
#         text = f'{class_name}: {iou_value:.2f}'
#         (text_width, text_height), baseline = cv2.getTextSize(text, cv2.FONT_HERSHEY_SIMPLEX, font_scale, thickness)
#         cv2.rectangle(img, (x1, y1 - text_height - baseline), (x1 + text_width, y1), color, cv2.FILLED)
#         cv2.putText(img, text, (x1, y1 - baseline), cv2.FONT_HERSHEY_SIMPLEX, font_scale, (255, 255, 255), thickness=thickness)
...
#         (text_width, text_height), baseline = cv2.getTextSize(text, cv2.FONT_HERSHEY_SIMPLEX, font_scale, thickness)
#         cv2.rectangle(img, (x1, y1 - 2 * (text_height + baseline)), (x1 + text_width, y1 - text_height - baseline), color, cv2.FILLED)
#         cv2.putText(img, text, (x1, y1 - baseline), cv2.FONT_HERSHEY_SIMPLEX, font_scale, (255, 255, 255), thickness=thickness)
...
if __name__ == '__main__':
    postfix = 'jpg'
    img_path = 'change_classes/images/test'
    label_path = 'change_classes/labels/test'
    predict_path = 'runs/detect_new/C2f_ODConv/labels'
    save_path = 'change_classes/save_pic'
    result_file_path = 'change_classes/save_txt/result.txt'
    classes = ['ab','normal']

    # BGR 绿色(检测正确)、红色(漏检)、蓝色(误检)
    ...
    with open(result_file_path, 'w') as f_w:
        for path in tqdm.tqdm(os.listdir(label_path)):
            ...
            for i in range(label.shape[0]):
                ...
                for j in ious_argsort:
                    ...
                    if label[i, 0] == pred[j][0]:
                        image = draw_box(image, pred[j][1:5], detect_color, classes[int(pred[j][0])], ious[j])
                        pred.pop(j)
                        missing = False
                        right_num += 1
                        break
                ...
                if missing:
                    image = draw_box(image, label[i][1:5], missing_color, classes[int(label[i][0])], 0)  # IoU设置为0
                    missing_num += 1
            ...
            if len(pred):
                for j in range(len(pred)):
                    class_index = int(pred[j][0])
                    class_name = classes[class_index] if class_index < len(classes) else "Unknown"
                    image = draw_box(image, pred[j][1:5], error_color, class_name)  # 不传递IoU值
                    error_num += 1
