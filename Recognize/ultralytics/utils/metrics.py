...
def plot_pr_curve(px, py, ap, save_dir=Path('pr_curve.png'), names=(), on_plot=None):
    ...
    ## 将结果保存至csv中 2024/2/26
    pr_dict = {'px': px.tolist()}  # Dictionary to hold precision-recall data for CSV
    # pr_dict = dict() 
    # pr_dict['px'] = px.tolist()
    ## 将结果保存至csv中 Edit by Lee

    if 0 < len(names) < 21:  # display per-class legend if < 21 classes
        for i, y in enumerate(py.T):
            ax.plot(px, y, linewidth=1, label=f'{names[i]} {ap[i, 0]:.3f}')  # plot(recall, precision)

            pr_dict[names[i]] = y.tolist() # 将结果保存至csv中 Edit by Lee--2024/2/26
    else:
    ...
    # ------------------- 将结果保存至csv中 Edit by Lee ---------------------- #
    # Save PR data to CSV
    save_csvpath = save_dir.with_suffix('.csv')  # Change .png to .csv for the file name
    pd.DataFrame(pr_dict).to_csv(save_csvpath, sep=',')

    interested_class = 'ab'
    # If interested in saving data for a specific class
    if interested_class and interested_class in names:
        class_index = names.index(interested_class)
        pr_dict_specific = {'px': px.tolist(), interested_class: py[:, class_index].tolist()}
        save_csvpath_specific = save_dir.with_name(f'{save_dir.stem}_{interested_class}').with_suffix('.csv')
        pd.DataFrame(pr_dict_specific).to_csv(save_csvpath_specific, sep=',')
    # ---------------------- Edit by Lee--2024/2/26 -------------------------- #
...
def plot_mc_curve(px, py, save_dir=Path('mc_curve.png'), names=(), xlabel='Confidence', ylabel='Metric', on_plot=None):
    ...
    # 初始化用于保存到CSV的字典
    mc_dict = {'px': px.tolist()}
    interested_class = 'ab'  # 您关注的指标
    interested_class_data = []  # 用于保存关注指标的数据

    # 是否存在您关注的指标
    interested_flag = False
    if interested_class in names:
        interested_index = names.index(interested_class)
    ...
    if 0 < len(names) < 21:  # display per-class legend if < 21 classes
        for i, y in enumerate(py):
           ...
           # # -----------------Lee edit: 将结果保存在csv中--------------- #
            mc_dict[names[i]] = y.tolist()  # 将每个类别的数据添加到字典中
            if names[i] == interested_class:
                interested_class_data = y.tolist()  # 保存关注指标的数据
           # # ------------------ Edit by Lee--2024/2/26 ----------------- #
    ...
    # -----------------Lee edit: 将结果保存在csv中--------------- #
    mc_dict['all'] = y.tolist()  # 添加整体指标数据到字典

    if interested_flag:
            mc_dict[interested_class] = interested_class_data  # 如果存在关注的指标，则添加到字典
    # if flag:
    ...
    # -----------------Lee edit: 将结果保存在csv中--------------- #
    import pandas as pd
    dataformat = pd.DataFrame(mc_dict)
    save_csvpath = save_dir.cwd() / (str(save_dir).replace('.png', '.csv'))  # 定义CSV文件的保存位置
    dataformat.to_csv(save_csvpath, sep=',')   
