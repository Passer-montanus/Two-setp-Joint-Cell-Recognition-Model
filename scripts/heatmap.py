...
class yolov8_heatmap:
    def __init__(self, weight, cfg, device, method, layer, backward_type, conf_threshold, ratio):
        ...
        # Custom colors for each class (BGR format)
        # Modify these colors as needed for your classes
        custom_colors = {
            0: (255, 0, 0),    # Class 0: Green
            1: (0, 255,0),    # Class 1: Red
            2: (0, 0, 255),    # Class 2: Blue
            # Add more classes as needed
        }
        
        # Create colors array with custom colors for known classes, random for others
        colors = np.zeros((len(model_names), 3), dtype=np.int)
        for i in range(len(model_names)):
            if i in custom_colors:
                colors[i] = custom_colors[i]
            else:
                colors[i] = np.random.uniform(0, 255, size=3).astype(np.int)
    ...
    def __call__(self, img_path, save_path):
        ...
        for i in trange(int(post_result.size(0) * self.ratio)):
            ...
            cam_image = self.draw_detections(post_boxes[i], self.colors[int(post_result[i, :].argmax())], f'{self.model_names[int(post_result[i, :].argmax())]}', cam_image) # 不包含阈值
...
if __name__ == '__main__':
    ...
    model(r'dataset_mixup/images/test/637.jpg', 'Heatmap/Final/637/14')